...
export async function POST() {
  try {
    ...
    const { data, error } = await supabaseAdmin.rpc("pgexec", { sql: schema })

    if (error) {
      console.error("Database setup error:", error)
      return NextResponse.json(
        {
          error: "Veritabanı kurulumu başarısız oldu",
    ...
    const { data: tablesCheck, error: tablesError } = await supabaseAdmin
      .from("books")
    ...
    if (tablesError) {
      return NextResponse.json(
        {
          error: "Tablolar oluşturuldu ancak veri kontrolü başarısız oldu",
          details: tablesError.message,
          output: "SQL çalıştırıldı ancak sonuç doğrulanamadı.",
        },
        { status: 500 },
      )
    }
    ...
    return NextResponse.json({
      ...
      output: "Tüm tablolar ve örnek veriler oluşturuldu.",
      tables: {
        books: "✓",
        categories: "✓",
    ...
    return NextResponse.json(
      {
        error: "Veritabanı kurulumu sırasında bir hata oluştu",
        details: error.message,
      },
      { status: 500 },
    )
